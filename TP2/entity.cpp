 ...
 * Date:		13/12/2020
 * File:		entity.cpp
 * Description:	Défénition des méthodes de entity
 */
...
{
	// init ints à 0
	_id = _type = _cost = _maxHP = _curHP = _range = _damage = _movement = _animationIterator = 0;
	_name = ""; // no name
...
void entity::setSprite(gameDataRef data, string spritename, int type, string behavior, string number)
{
	string path;
	// change le filepath dépendant du type d'entity
	switch (type) {
	case 1:
		path = "Resources/res/entitySprites/unit_" + spritename + "_" + behavior + "_" + number + ".png";
		break;
	case 2:
		path = "Resources/res/entitySprites/mob_" + spritename + "_" + behavior + "_" + number + ".png";
		break;
	}

	string textureName = spritename + behavior + number;
	data->assets.loadTexture(textureName, path);
	_sprite.setTexture(data->assets.getTexture(textureName));
}

void entity::setSpriteName(string spriteName)
{

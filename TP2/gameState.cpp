...
void gameState::init()
{
	...
	_hearth.setPosition(SCREEN_WIDTH - _enemyZone.getGlobalBounds().width - 75,85-_hearth.getGlobalBounds().height/2);

	// load hearth text
	_hearthText.setFont(_data->assets.getFont("game font"));
	_hearthText.setString(to_string(_playerHP));
	_hearthText.setCharacterSize(36);
	_hearthText.setFillColor(Color::Red);
	_hearthText.setOrigin(_hearthText.getGlobalBounds().width / 2, _hearthText.getGlobalBounds().height / 2);
	_hearthText.setPosition(_hearth.getPosition().x + _hearth.getGlobalBounds().width + 15, 75);
...
void gameState::draw(float dt) const
{
	...
	//playerHP
	_data->window.draw(_hearth);
	_data->window.draw(_hearthText);
